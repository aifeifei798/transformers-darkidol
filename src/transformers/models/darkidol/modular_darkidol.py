...
class DarkidolConfig(PretrainedConfig):
    ...
    def __init__(
        self,
        text_config: Optional[Union[DarkidolTextConfig, dict[str, Any]]] = None,
        ...
        **kwargs,
        ...
        if text_config is None:
            text_config = DarkidolTextConfig()
            logger.info("text_config is None, using default DarkidolTextConfig text config.")
        elif isinstance(text_config, dict):
            text_config = DarkidolTextConfig(**text_config)
...
class DarkidolModelOutputWithPast(PaligemmaModelOutputWithPast):
    pass


class DarkidolCausalLMOutputWithPast(PaligemmaCausalLMOutputWithPast):
    pass


class DarkidolTextScaledWordEmbedding(nn.Embedding):
    """
...
class DarkidolAttention(Gemma2Attention):
    def __init__(self, config: DarkidolTextConfig, layer_idx: int):
        ...
        self.q_norm = DarkidolRMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
        self.k_norm = DarkidolRMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
...
class DarkidolDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: DarkidolTextConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.self_attn = DarkidolAttention(config=config, layer_idx=layer_idx)
        self.mlp = DarkidolMLP(config)
        self.input_layernorm = DarkidolRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = DarkidolRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = DarkidolRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = DarkidolRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class DarkidolTextModel(Gemma2Model):
    ...
    def __init__(self, config: DarkidolTextConfig):
        ...
        # Darkidol downcasts the below to bfloat16, causing sqrt(3072)=55.4256 to become 55.5. See https://github.com/huggingface/transformers/pull/29402
        self.embed_tokens = DarkidolTextScaledWordEmbedding(
        ...
        self.rotary_emb_local = DarkidolRotaryEmbedding(config=config)
...
class DarkidolForCausalLM(Gemma2ForCausalLM):
    ...
    def __init__(self, config: DarkidolTextConfig):
        super().__init__(config)
        self.model = DarkidolTextModel(config)


class DarkidolMultiModalProjector(nn.Module):
    def __init__(self, config: DarkidolConfig):
        super().__init__()
        ...
        self.mm_soft_emb_norm = DarkidolRMSNorm(
...
class DarkidolModel(PaliGemmaModel):
    ...
    def forward(
        ...
        **lm_kwargs,
    ) -> Union[tuple, DarkidolModelOutputWithPast]:
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError("You must specify exactly one of input_ids or inputs_embeds")
        ...
        return DarkidolModelOutputWithPast(
...
class DarkidolForConditionalGeneration(PaliGemmaForConditionalGeneration):
    ...
    def forward(
        ...
        **lm_kwargs,
    ) -> Union[tuple, DarkidolCausalLMOutputWithPast]:
        r"""
        ...
        return DarkidolCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
class DarkidolForSequenceClassification(DarkidolPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = DarkidolModel(config)
...
__all__ = [
    "DarkidolConfig",
    "DarkidolTextConfig",
    "DarkidolPreTrainedModel",  # noqa: F822
    "DarkidolTextModel",
    "DarkidolForCausalLM",
    "DarkidolForConditionalGeneration",
    "DarkidolModel",
    "DarkidolForSequenceClassification",
]
