...
python -m transformers.models.darkidol.convert_darkidol_weights_orbax_to_hf \
    --variant='darkidol_4b' \
    --tokenizer_path="$HOME/darkidol/tokenizer/darkidol_cleaned_262144_v2.spiece.model" \
    --checkpoint_path="$HOME/darkidol/darkidol_4b_pt_orbax/" \
    --output_path="$HOME/darkidol/darkidol_4b_pt_safetensors/"
"""
...
def convert(checkpoint_path: str, config: DarkidolConfig) -> dict[str, torch.Tensor]:
...
def main(*args):
    ...
    if variant != _VARIANT_Darkidol_1B:
        ...
        processor = DarkidolProcessor(
            image_processor=image_processor,
            tokenizer=tokenizer,
            chat_template=tokenizer.chat_template,
        )
        processor.save_pretrained(output_path)
        logging.info("Saved DarkidolProcessor for %s to %s", variant, output_path)
        del processor
